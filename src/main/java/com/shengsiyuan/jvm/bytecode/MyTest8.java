 ...
 *
 * 所谓解释执行，就是通过解释器来读取字节码，遇到相应的指令就会执行该指令。
 ...
 * 4. 基于栈的指令集的缺点在于完成相同的操作，指令数量通常比基于寄存器的指令数量要多；基于栈的指令集是在内存中完成操作的，
 *    而基于寄存器的指令集是直接有CPU来执行的，它是在高速缓存区中进行执行的，速度要快很多。虽然虚拟机可以采用一些优化手段，
